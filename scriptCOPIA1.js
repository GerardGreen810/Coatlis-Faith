...
function animarMovimiento() {
  ...
  if (teclas.a) {
    ...
    // ¿puede moverse el fondo hacia la derecha? (fondoX < limiteIzq -> aún no está en 0)
    if (fondoX < limiteIzq) {
      fondoX = Math.min(limiteIzq, fondoX + velocidad);
      // si el personaje estaba desplazado hacia la izquierda, empujarlo de vuelta al centro
      if (posPersonaje < centerPercent) {
        posPersonaje = Math.min(centerPercent, posPersonaje + retornoSpeed);
      }
    } else {
      // fondo ya tocó su extremo derecho (fondoX == limiteIzq). Ahora permitimos que el personaje avance hacia la izquierda
    ...
    // ¿puede moverse el fondo hacia la izquierda? (fondoX > limiteDer -> aún no llegó al máximo negativo)
    if (fondoX > limiteDer) {
      fondoX = Math.max(limiteDer, fondoX - velocidad);
      // si el personaje estaba desplazado hacia la derecha, empujarlo de vuelta al centro
      if (posPersonaje > centerPercent) {
        posPersonaje = Math.max(centerPercent, posPersonaje - retornoSpeed);
      }
    } else {
      // fondo ya tocó su extremo izquierdo (fondoX == limiteDer). Ahora permitimos que el personaje avance hacia la derecha
